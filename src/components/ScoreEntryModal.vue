...
<template>
  <BaseModal
    ...
    @close="emit('close')"
  >
    <div class="mb-4 flex flex-wrap items-center gap-3 text-sm text-[#5d5b54] dark:text-slate-400">
      <span>
        {{
          match.stage === 'group'
    ...
    <h4 class="mb-2 text-sm font-bold text-[#5d5b54] dark:text-slate-400">
      各局成绩（相对标准杆，如 -14）
    </h4>
      ...
      <table class="notion-table w-full text-sm">
        ...
        <tbody>
          <tr
            ...
            :key="index"
            class="border-b border-[#ede9e4] dark:border-[#4a426e]"
          >
            ...
            <td :data-label="`${playerA?.name || '甲'} 相对标准杆`" class="py-2 pr-2">
              <input
                ...
                type="number"
                placeholder="如 -14"
                class="w-24 rounded-sm border border-[#c8c4be] px-2 py-1.5 dark:border-[#675d8e] dark:bg-[#4d4778]"
              />
            </td>
            ...
            <td data-label="平局 SD 胜者" class="py-2">
              <select
                ...
                class="w-32 rounded-sm border border-[#c8c4be] px-2 py-1.5 pr-7 dark:border-[#675d8e] dark:bg-[#4d4778] disabled:cursor-not-allowed disabled:opacity-50"
              >
    ...
    <h4 class="mb-2 text-sm font-bold text-[#5d5b54] dark:text-slate-400">结果截图链接</h4>
    <div class="mb-4">
      <div class="mb-2 flex flex-wrap gap-2">
        <input
          ...
          class="flex-1 rounded-sm border border-[#c8c4be] px-3 py-2 dark:border-[#675d8e] dark:bg-[#4d4778]"
        />
    ...
    <div
      ...
      class="notion-tint-yellow mb-4 rounded-xl p-4 dark:bg-amber-900/20"
    >
      <div class="mb-2 grid gap-2 sm:grid-cols-2">
        <div>
          ...
          <select
            ...
            class="w-full rounded-sm border border-[#c8c4be] px-2 py-1.5 pr-8 dark:border-[#675d8e] dark:bg-[#4d4778]"
          >
      ...
      <p class="mb-2 text-xs text-[#793400] dark:text-amber-400">
        剩余需重赛洞数：{{ 9 - (form.disconnect.holesCompleted || 0) }}（结果合并计算）
      </p>
      <textarea
        v-model="form.disconnect.note"
        placeholder="掉线情况说明"
        class="mb-2 w-full rounded-sm border border-[#c8c4be] px-3 py-2 text-sm dark:border-[#675d8e] dark:bg-[#4d4778]"
      ></textarea>
    ...
    <div
      class="mb-4 rounded-xl bg-[#e5f6ea] p-4 text-sm dark:bg-emerald-900/20"
    >
      <p class="font-semibold text-[#1aae39] dark:text-emerald-400">
        判定预览：
    ...
    <p v-if="success" class="mb-3 rounded-lg bg-[#e5f6ea] px-3 py-2 text-sm text-[#1aae39] dark:bg-emerald-900/30">
      {{ success }}
    </p>
