...
基于《NSS鬼吃鱼高尔夫锦标赛比赛规则》的 16 人高尔夫锦标赛赛况记录网站：从抽签分组、小组赛积分、淘汰赛对阵，到赛果与证据管理，一站完成。管理员口令登录后可编辑，选手与游客只读。
...
- 积分榜：四组同页展示，按规则自动排序（积分 → 相互战绩 → 净胜局 → 净胜杆 → 组织方抽签）
...
- 管理员口令不内置在代码中：本地回退模式在 `.env` 的 `VITE_ADMIN_PASSWORD` 里配置（该文件已被 git 忽略，不会提交）；启用 Supabase 后改用 Supabase 管理员账号密码登录。

## Supabase 配置

1. 在 Supabase Dashboard → **SQL Editor** 执行 `supabase/schema.sql`（建表 + RLS：游客只读、登录用户可写）。
2. **Authentication → Users → Add user** 创建管理员账号（勾选 Auto Confirm User）。
...
> 云端首次写入会在管理员登录后自动触发；本地已有数据会同步到云端。

## 数据与重置

- 初始预设：16 名选手（含头像）、按历史最佳分档、**未抽签**，等待组织方抽签发布。
- 管理后台「选手与分组 → 重置赛事」可清空分组与赛程（保留选手名单）。
...
- 排名：积分 → 相互战绩 → 净胜局 → 净胜杆 → 组织方抽签。
- 淘汰赛：BO5 先 3 胜，固定对阵，每周 1 场，DDL 由组织方公布。
